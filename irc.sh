...
. pgrep.inc.sh
. irc.inc.sh
[[ $IRC_NICK == "ec" ]] && . upvars.inc.sh
[[ $IRC_NICK == "ec" ]] && . array.class.inc.sh
...
function ips() {
	# inet 174.127.117.151/24 brd 174.127.117.255 scope global secondary eth0:18
	# ip addr show | grep 'inet.* eth' | grep -oP '\d+\.\d+\.\d+\.\d+'
...
function pplproxy_fail
{
        echo "pplproxy failed check: $*"
        killall -TERM pplproxy
		  launch pplproxy
		  return
		  ...
		  {
			  echo "/root/pplproxy not found"
		  }
		  exit 0
}
function pplnet_fail
{
        echo "pplnet failed check: $*"
        killall -TERM pplnet
		  launch pplnet
		  return
		  ...
		  {
			  echo "/root/pplnet not found"
			  exit 1
		  }

		  sleep 1
		  pidof pplnet > /dev/null && echo "pplnet launched" || echo "pplnet failed"
}


function check_pplnet
{
	valid pplnet || return
	pidof pplnet > /dev/null || pplnet_fail "pplnet not running"
	lsof -i4:9595 > /dev/null || pplnet_fail "pplnet not listening"
	return
	export IP=$( curl --silent http://ip.nt4.com )
	RESULT=$( curl --silent --max-time 8 --proxy-user pplproxy:$IP --socks5-hostname spanky2.nt4.com:9596 http://ip.nt4.com/ 2>&1 )
	el=$?
	(( el )) && pplnet_fail "pplnet/curl fail $el" 
	[[ ! $RESULT =~ $IP ]] && pplnet_fail "curl did not return expected ip '$IP' instead: '$RESULT'"
}
function check_pplproxy
{
	test -x /root/src/pplnet/pplproxy && echo 'LD_LIBRARY_PATH=/usr/local/lib /root/src/pplnet/pplproxy' > /root/pplproxy && chmod 755 /root/pplproxy
	test -x /root/src/pplnet/pplproxy || echo "/root/src/pplnet/pplproxy not found"
	valid pplproxy || return 
	pidof pplproxy > /dev/null || pplproxy_fail "pplproxy not running"
	lsof -i4:59595 > /dev/null || pplproxy_fail "pplproxy not listening"
	IP=$( curl --silent ip.nt4.com )
	RESULT=$( curl --silent --max-time 5 --proxy-user pplproxy:$IP --socks5 203.26.88.254:59596 ip.nt4.com )
	(( $? )) && pplproxy_fail "curl returned with errorlevel $el"
	[[ ! $RESULT =~ $IP ]] && pplproxy_fail "curl did not return expected ip '$IP' instead: '$RESULT'"
}
...
# Ops
# [ MODE ] :user!user@rox-E4FDA80C.gqle1.lon.bigpond.net.au MODE #proxies +o ec
onMODE() {
	CHANNEL=$3
	MODE=$4
	NICK=$5
...
# [ JOIN ] :user!user@rox-E4FDA80C.gqle1.lon.bigpond.net.au JOIN :#proxies
onJOIN() {
...
whois() {
	write "WHOIS user"
	# [ 313 ] :irc5.foonet.com 313 ec user :is a Network Administrator
}
...
known_bots="c30 c35 c36 c37 c38 c39 chips10 chips13 chips16 chips20 chips26 chips3 chips30 chips35 chips5 chips7 chips9 mailer_daemon sgstinky stinky2 stinky3 vpn"
for bot in $known_bots
do
	knownBots["$bot"]="$bot"
	missingBots["$bot"]="$bot"
...
onNOTICE() {
	# :user!user@rox-25E0BD4.gqle1.lon.bigpond.net.au NOTICE #wwii :This is a notice, you will be ponged to death.
	PREFIX="$1"
...
onPRIVMSG() {
	# [ PRIVMSG ] :user!user@rox-E4FDA80C.gqle1.lon.bigpond.net.au PRIVMSG #proxies :hello idiot
	# [ PRIVMSG ] :user!user@rox-E4FDA80C.gqle1.lon.bigpond.net.au PRIVMSG ec :hello idiot
	# [ PRIVMSG ] :user!user@rox-E4FDA80C.gqle1.lon.bigpond.net.au PRIVMSG ec :memory
	# PRIVMSG: "memory"
	...
	case "${CMD}" in
		...
		launch )		if [ -f /usr/src/zillow/vps/vps_process.php ]
								...
								for host in ${missingBots[@]}; do
									...
									(( ${PIPESTATUS[0]} == 2 )) && {
										...
										if (( ${PIPESTATUS[0]} == 2 )); then
											...
											### php vps.php --power-off --power-on --host "${host}" &
						...
						;;
		upgrade )	# svn --username user --password password upgrade /root/proxies 2>&1 | pipe_to_privmsg
						# svn --username user --password password revert -R /root/proxies 2>&1 | head -n20 | pipe_to_privmsg
						# svn --username user --password password up /root/proxies 2>&1 | head -n20 | pipe_to_privmsg

						# root@chips19:~/.subversion# vi servers # store-plaintext-passwords = no
						# remove leading # from ~/.subversion/servers 
						# echo -n svn1; yes yes | svn --username user --password password --non-interactive --force upgrade /root/proxies 2>&1 2>&1 | head -n10
						#### echo -n svn2 " "; svn --username user --password password --non-interactive revert -R /root/proxies 2>&1 | head -n10
						#### echo -n svn3 " "; svn --username user --password password --non-interactive up /root/proxies 2>&1 | head -n10
						(
						...
						svn --username user --password password up /root/proxies
						#						svn up /root/proxies || {
						#							svn upgrade /root/proxies && svn up /root/proxies || {
						#							}
						#						}
						) | pipe_to_privmsg
						
						# echo -n svn3; yes yes | svn --username user --password password --non-interactive revert -co https://user.svn.cvsdude.com/wrangler/proxies /root/proxies 2>&1 | head -n10
						# echo -n svn4; yes yes | svn up /root/proxies 2>&1 | head -n10
						# yes yes | svn --username user --password password co https://user.svn.cvsdude.com/wrangler/proxies /root/proxies 2>&1 | head -n20 | pipe_to_privmsg
						# --accept theirs-full 
						# --force
...
#						{
							# test -e /home/user/wrangler/addcomment && /home/user/wrangler/addcomment "irc.sh: restarted $app"
							killall -HUP $app
